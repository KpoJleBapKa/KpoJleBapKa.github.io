...
function addLesson(lesson) {
  ...
  schedule.push(
    Object.assign(Object.assign({}, lesson), { id: lessonIdCounter++ }),
  );
  return true;
}
...
function reassignClassroom(lessonId, newClassroomNumber) {
  ...
  const conflict = validateLesson(
    Object.assign(Object.assign({}, lesson), {
      classroomNumber: `Room ${newClassroomNumber}`,
    }),
    lessonId,
  );
  if (conflict) {
...
(_a = document.getElementById("professorForm")) === null || _a === void 0
  ? void 0
...
(_b = document.getElementById("courseForm")) === null || _b === void 0
  ? void 0
...
(_c = document.getElementById("lessonForm")) === null || _c === void 0
  ? void 0
  : _c.addEventListener("submit", function (event) {
      event.preventDefault();
      const courseId = parseInt(
        document.getElementById("lessonCourseId").value,
      );
      const professorId = parseInt(
        document.getElementById("lessonProfessorId").value,
      );
      const classroomNumber = document.getElementById(
        "lessonClassroomNumber",
      ).value;
...
(_d = document.getElementById("professorScheduleForm")) === null ||
_d === void 0
  ? void 0
  : _d.addEventListener("submit", function (event) {
      ...
      document.getElementById("scheduleResult").innerText =
        `Professor's Schedule: ${professorLessons.map((lesson) => `Lesson ID: ${lesson.id}, Course ID: ${lesson.courseId}, Classroom: ${lesson.classroomNumber}, Day: ${lesson.dayOfWeek}, Time: ${lesson.timeSlot}`).join("; ")}`;
    });
// взаємодія з DOM для обчислення використання аудиторій
(_e = document.getElementById("classroomUtilizationForm")) === null ||
_e === void 0
  ? void 0
  : _e.addEventListener("submit", function (event) {
      event.preventDefault();
      const utilization = getClassroomUtilization();
      document.getElementById("utilizationResult").innerText =
        `Classroom utilization: ${utilization.toFixed(2)}%`;
    });
// взаємодія з DOM для пошуку доступних аудиторій
(_f = document.getElementById("availableClassroomsForm")) === null ||
_f === void 0
  ? void 0
  : _f.addEventListener("submit", function (event) {
      event.preventDefault();
      ...
      document.getElementById("availableClassroomsResult").innerText =
        `Available Classrooms: ${availableClassrooms.join(", ")}`;
    });
// взаємодія з DOM для визначення найпопулярнішого типу курсу
(_g = document.getElementById("popularCourseTypeForm")) === null ||
_g === void 0
  ? void 0
  : _g.addEventListener("submit", function (event) {
      event.preventDefault();
      const popularType = getMostPopularCourseType();
      document.getElementById("popularCourseTypeResult").innerText =
        `Most Popular Course Type: ${popularType}`;
    });
// взаємодія з DOM для перепризначення аудиторії
(_h = document.getElementById("reassignClassroomForm")) === null ||
_h === void 0
  ? void 0
  : _h.addEventListener("submit", function (event) {
      event.preventDefault();
      const lessonId = parseInt(
        document.getElementById("reassignLessonId").value,
      );
      ...
      if (reassignClassroom(lessonId, newClassroomNumber)) {
        document.getElementById("reassignResult").innerText =
          "Classroom reassigned successfully.";
      } else {
        document.getElementById("reassignResult").innerText =
          "Failed to reassign classroom due to conflict.";
      }
    });
// взаємодія з DOM для видалення заняття
(_j = document.getElementById("cancelLessonForm")) === null || _j === void 0
  ? void 0
  : _j.addEventListener("submit", function (event) {
      event.preventDefault();
      const lessonId = parseInt(
        document.getElementById("cancelLessonId").value,
      );
