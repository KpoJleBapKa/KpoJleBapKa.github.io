...
function validateLesson(lesson: Omit<Lesson, 'id'>, excludeLessonId?: number): ScheduleConflict | null {
...
function reassignClassroom(lessonId: number, newClassroomNumber: string): boolean {
    ...
    const conflict = validateLesson({ ...lesson, classroomNumber: `Room ${newClassroomNumber}` }, lessonId); // перевіряємо на конфлікти
    if (conflict) {
...
function getMostPopularCourseType(): CourseType | null {
    ...
    return Object.keys(typeCount).reduce((a, b) => typeCount[a as CourseType] > typeCount[b as CourseType] ? a : b) as CourseType; // знаходимо найпопулярніший тип
}
...
document.getElementById('lessonForm')?.addEventListener('submit', function(event) {
    event.preventDefault(); // запобігаємо стандартній поведінці форми
    const courseId = parseInt((document.getElementById('lessonCourseId') as HTMLInputElement).value); // отримуємо ID курсу
    const professorId = parseInt((document.getElementById('lessonProfessorId') as HTMLInputElement).value); // отримуємо ID професора
